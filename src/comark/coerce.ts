...
/** Prop names that a given component wants as numbers. */
...
export function coerceProps<T extends Record<string, unknown>>(
  ...
  for (const [rawKey, rawValue] of Object.entries(props)) {
    // Comark meta (`$` bag) and slot plumbing are not component props.
    if (rawKey.startsWith("$")) continue

    const unprefixed = rawKey.startsWith(":") ? rawKey.slice(1) : rawKey

    // Comark emits `class` for the `{.foo}` shorthand and for `class:` props,
    // which is what an author would reach for to size a figure. The graph
    // components take React's `className`.
